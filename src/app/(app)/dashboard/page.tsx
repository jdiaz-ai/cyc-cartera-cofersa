...
interface GestionRow   { id: string; cliente_cod: string; tipo: string; resultado: string; hora: string; analista_email: string }
...
export default async function DashboardPage() {
  ...
  // Promesas que vencen esta semana (próximos 7 días, excluyendo hoy)
  let promSemana = 0
  try {
    const fin = new Date(Date.now() - 6 * 3600000)
    fin.setDate(fin.getDate() + 7)
    const semFin = fin.toISOString().split('T')[0]
    const { count } = await supabase.from('promesas').select('*', { count: 'exact', head: true })
      .eq('estado', 'PENDIENTE').gt('fecha_promesa', hoyStr).lte('fecha_promesa', semFin)
    promSemana = count ?? 0
  } catch {}
  ...
  // ── Mi Equipo — analistas + gestiones de hoy ─────────────────────────
  let analistas: AnalistaRow[] = []
  let equipoStats: { id: string; nombre: string; iniciales: string; color: string; gHoy: number }[] = []
  try {
    const { data } = await supabase.from('usuarios')
      .select('id,nombre,email,iniciales,color')
      .eq('rol', 'ANALISTA').eq('activo', true)
    analistas = (data ?? []) as AnalistaRow[]

    // Gestiones de hoy de todo el equipo — una sola query
    const { data: gData } = await supabase.from('gestiones')
      .select('analista_email').eq('fecha', hoyStr)
    const conteo: Record<string, number> = {}
    for (const g of (gData ?? []) as { analista_email: string }[]) {
      conteo[g.analista_email] = (conteo[g.analista_email] || 0) + 1
    }
    equipoStats = analistas.map(a => ({
      id: a.id, nombre: a.nombre, iniciales: a.iniciales, color: a.color,
      gHoy: conteo[a.email] ?? 0,
    }))
  } catch {}

  const avgEquipo = equipoStats.length
    ? Math.round(equipoStats.reduce((s, a) => s + a.gHoy, 0) / equipoStats.length)
    : 0
  const maxEquipo = Math.max(...equipoStats.map(a => a.gHoy), 1)

  // Mapa email→nombre para el feed de gestiones
  ...
  // ── Alertas automáticas ───────────────────────────────────────────────
  // Ordenadas: ROJO primero → AMARILLO → CYAN
  const alertas: Alerta[] = []
  if (solicPendientes > 0)
    alertas.push({ nivel: 'ROJO', texto: `${solicPendientes} solicitud${solicPendientes > 1 ? 'es' : ''} pendiente${solicPendientes > 1 ? 's' : ''} de revisión` })
  if (promVencidas.length > 0)
    alertas.push({ nivel: 'ROJO', texto: `${promVencidas.length} promesa${promVencidas.length > 1 ? 's' : ''} vencieron hoy sin confirmar` })
  if (promSemana > 0)
    alertas.push({ nivel: 'AMARILLO', texto: `${promSemana} promesa${promSemana > 1 ? 's' : ''} vencen esta semana` })
  if (gHoy === 0)
    alertas.push({ nivel: 'AMARILLO', texto: 'Sin gestiones registradas hoy' })
  if (alertas.length === 0)
    alertas.push({ nivel: 'CYAN', texto: 'Todo al día — sin alertas críticas' })
  ...
  const alertaColor: Record<NivelAlerta, { dot: string; bg: string; text: string }> = {
    ...
    <div className="min-h-full" style={{ background: '#EEF2F7' }}>
      <div className="px-6 pt-5 pb-6 space-y-5">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-6 gap-3">
          ...
          <KPICard label="Total en Mora"       valor={fmtM(mora)}     sub={`${nMora.toLocaleString()} clientes`}                                            gradient={pMora>20?"linear-gradient(135deg,#991b1b,#dc2626)":"linear-gradient(135deg,#065f46,#059669)"} badge={`${pMora}%`} badgeGood={pMora<=20} icon={<TrendingDown size={16}/>}  />
          ...
          <KPICard label="Clientes Activos"    valor={String(nClientes).replace(/\B(?=(\d{3})+(?!\d))/g,'.')} sub={`${nMora.toLocaleString()} con mora`}    gradient="linear-gradient(135deg,#1d4ed8,#3b82f6)"    badge={null}                icon={<Users size={16}/>}        />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          ...
          <div className="xl:col-span-2" style={{ background:'white', borderRadius:'16px', border:'1px solid #E2E8F0', boxShadow:'0 1px 8px rgba(0,0,0,0.06)', overflow:'hidden' }}>
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom:'1px solid #F1F5F9' }}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background:'rgba(0,59,92,0.08)' }}>
                  <Activity size={15} style={{ color:'#003B5C' }}/>
                </div>
                <div>
                  <h2 className="text-sm font-bold text-gray-900">Distribución de Cartera · Aging</h2>
                  <p className="text-xs text-gray-400">6 tramos · {fechaCorte}</p>
                </div>
              </div>
              ...
              {aging.map(t => {
                ...
                return (
                  <div key={t.label} className="rounded-xl px-4 py-2.5 flex items-center gap-4" style={{ background:t.bg, border:`1px solid ${t.border}` }}>
                    ...
                    <div className="flex-1 h-1.5 rounded-full" style={{ background:'rgba(255,255,255,0.6)' }}>
                      <div className="h-full rounded-full" style={{ width:`${Math.max(barW,2)}%`, background:t.color }}/>
                    </div>
            ...
            <div className="px-6 py-3 flex flex-wrap items-center gap-6" style={{ background:'#F8FAFC', borderTop:'1px solid #F1F5F9' }}>
              <div className="flex items-center gap-2"><span className="text-xs font-bold text-gray-400 uppercase tracking-wider">DSO</span><span className={`text-sm font-black ${dso>40?'text-red-600':'text-green-600'}`}>{dso} días</span><span className="text-xs text-gray-300">/&lt;40d</span></div>
              <div className="flex items-center gap-2"><span className="text-xs font-bold text-gray-400 uppercase tracking-wider">% Mora</span><span className={`text-sm font-black ${pMora>20?'text-red-600':'text-green-600'}`}>{pMora}%</span><span className="text-xs text-gray-300">/&lt;20%</span></div>
          ...
          <div style={{ background:'white', borderRadius:'16px', border:'1px solid #E2E8F0', boxShadow:'0 1px 8px rgba(0,0,0,0.06)', overflow:'hidden' }}>
            <div className="px-5 py-4 flex items-center gap-3" style={{ borderBottom:'1px solid #F1F5F9' }}>
              ...
              <div>
                <h2 className="text-sm font-bold text-gray-900">Alertas</h2>
                <p className="text-xs text-gray-400">{alertas.filter(a=>a.nivel==='ROJO').length} críticas · {alertas.filter(a=>a.nivel==='AMARILLO').length} de atención</p>
              </div>
            </div>
            <div className="p-4 space-y-2.5">
              {alertas.map((a, i) => {
                const cfg = alertaColor[a.nivel]
                return (
            ...
            {/* Promesas vencidas detalle */}
            {promVencidas.length > 0 && (
              <div className="px-4 pb-4 space-y-1.5">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider px-1 mb-2">Detalle promesas vencidas</p>
                {promVencidas.map(p => (
            ...
            {meta > 0 && (
              <div className="mx-4 mb-4 rounded-xl p-4" style={{ background:'linear-gradient(135deg,#003B5C,#005a8e)' }}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold uppercase tracking-widest text-blue-300">Meta Mensual</p>
                  <CheckCircle2 size={13} className="text-blue-400"/>
                </div>
                <p className="text-white text-lg font-black">{fmtM(meta)}</p>
                <p className="text-blue-400 text-xs mt-1">Módulo de pagos pendiente de activar</p>
              </div>
            )}
          </div>
        </div>

        {/* ── Fila 3: Mi Equipo (1/2) + Gestiones Recientes (1/2) ── */}
            ...
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom:'1px solid #F1F5F9' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Actividad de hoy · promedio {avgEquipo} gestiones</p>
                </div>
              </div>
              <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background:'rgba(0,59,92,0.08)', color:'#003B5C' }}>
                {equipoStats.length} analistas
              </span>
            </div>
            <div className="p-4 space-y-3">
              {equipoStats.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-sm font-semibold text-gray-400">Sin analistas activos</p>
                </div>
              ) : (
                equipoStats
                  .sort((a, b) => b.gHoy - a.gHoy)
                  .map(a => {
                    const barW = maxEquipo > 0 ? Math.round((a.gHoy / maxEquipo) * 100) : 0
                    const sobreAvg = avgEquipo > 0 && a.gHoy >= avgEquipo
                    return (
                      <div key={a.id} className="flex items-center gap-3">
                        {/* Avatar */}
                        <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-white font-bold text-xs" style={{ backgroundColor: a.color }}>
                          {a.iniciales}
                        </div>
                        {/* Nombre + barra */}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className={`text-xs font-black ml-2 flex-shrink-0 ${sobreAvg ? 'text-green-600' : a.gHoy === 0 ? 'text-gray-300' : 'text-amber-600'}`}>
                              {a.gHoy}
                            </span>
                          </div>
                          <div className="h-1.5 rounded-full overflow-hidden" style={{ background:'#F1F5F9' }}>
                            <div
                              ...
                              style={{ width:`${Math.max(barW, a.gHoy > 0 ? 4 : 0)}%`, background: sobreAvg ? '#16a34a' : a.gHoy === 0 ? '#E2E8F0' : '#f59e0b' }}
                            />
                          </div>
                        </div>
                      </div>
                  ...
                  <div key={g.id} className="rounded-xl px-3 py-2.5 flex items-center gap-3" style={{ background:'#F8FAFC' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {g.analista_email && emailNombre[g.analista_email] && (
                          <span className="text-xs text-gray-400 flex-shrink-0">· {emailNombre[g.analista_email]}</span>
                        )}
                      </div>
    ...
    <div style={{ background:gradient, borderRadius:'16px', boxShadow:'0 4px 24px rgba(0,0,0,0.14)', overflow:'hidden' }} className="p-5 relative">
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-between mt-2">
          <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'11px' }} dangerouslySetInnerHTML={{ __html: sub }}/>
          {badge && (
            <span style={{ background: badgeGood?'rgba(74,222,128,0.25)':'rgba(255,100,100,0.25)', color: badgeGood?'#86efac':'#fca5a5', fontSize:'10px', fontWeight:700, padding:'2px 8px', borderRadius:'20px', textTransform:'uppercase' }}>
              {badge}
            </span>
          )}
        </div>
